...
- В боте включено постоянное командное меню (нижняя клавиатура): `/start`, `/web`, `/credits`, `/buy`, `/status`, `/help`.
- Команда `/web` отправляет персональную ссылку с автоподстановкой `client_token`.
