...
- `/start` показывает быстрый сценарий и inline-кнопки: `Генерировать в боте` / `Открыть веб`.
- В `/start` добавлено приветствие и быстрый выбор режима работы (бот/веб).
- Команда `/web` отправляет персональную ссылку с автоподстановкой `client_token` (если токена нет — бот создаст его автоматически).
- В web `client_token` можно передать в URL (`?client_token=...`): токен сохраняется в `localStorage`, затем убирается из адресной строки.
...
- `/start`, `/help`
- `/web` — открыть web с уже подставленным `client_token`
- `/status`
- `/myid`
- `/credits`
- `/buy`
...
- `/start` — показывает быстрые кнопки: «Генерировать в боте» и «Открыть веб».
